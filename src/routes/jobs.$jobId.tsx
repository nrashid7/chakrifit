...
function JobDetail() {
  ...
  if (job.isLoading) return <Shell><p className="text-center py-12 text-muted-foreground">Loading…</p></Shell>;
  if (!job.data?.job) return <Shell><p className="text-center py-12">Job not found.</p></Shell>;
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {!isAuthed && (
          <div className="rounded-2xl border bg-card p-6 text-center">
            <p className="text-muted-foreground">Sign in to see your match score and save this job.</p>
            <Link to="/login" className="inline-block mt-3"><Button>Sign in</Button></Link>
          </div>
        )}
      </div>
    </Shell>
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen flex flex-col">
      <SiteHeader right={<Link to="/dashboard"><Button variant="ghost" size="sm">Dashboard</Button></Link>} />
